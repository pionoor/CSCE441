...
class Interpolator
{
	...
	public: 
		...
		Interpolator(Motion* pInitialMotion, std::list<int> pOffsetFileName, InterpType type);
		~Interpolator();
		...
		void ReadOffset(std::list<int> pOffsetFileName);
	...
	private:
		...
		void CatMullInterp(Motion* pInterpMotion);
	...
	private:
		...
		std::vector<int> m_pTimeDistArray;			//Array containing information on the time distance (number of frames) 
