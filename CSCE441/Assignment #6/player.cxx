...
static Interpolator *interp = NULL;

static list<int> selectedFrame;

static int nFrameNum, nFrameInc=1;		// Current frame and frame increment
static bool actorLoaded = false;
static bool motionLoaded = false;
...

{
	if (selectedFrame.size() < 4) {
		cout << "Select at least 4 frames!" << endl;
		return;
	}

	interp = new Interpolator(pSampledMotion, selectedFrame, InterpType::CATMULL);
	interp->Interpolate(pInterpMotion);
...
void loadOriginal_callback(Fl_Button *button, void *){
...
void select_callback(Fl_Button *button, void *){

	if (actorLoaded && motionLoaded){
		selectedFrame.push_back(nFrameNum + 1);
	}
	else{
		cout << "the actor or the motion files has not been loaded!" << endl;
	}
}
